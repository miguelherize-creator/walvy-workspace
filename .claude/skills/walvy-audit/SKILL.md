...
- **back-walvy** — NestJS 11 + TypeORM + PostgreSQL. Rama de integración: `qa`.
- **workspace/walvy-workspace** — memoria. Skills invocables en `.claude/skills/`.
...
3. **Elige la lente** según el archivo: frontend RN/Expo → aplica `.claude/skills/senior-react-native/SKILL.md`; backend NestJS → `.claude/skills/walvy-backend-auditor/SKILL.md`; fidelidad visual contra Figma → `.claude/skills/ui-visual-qa/SKILL.md`.
...
cd back-walvy && pnpm run preview:email   # → src/mail/previews/*.html
```
