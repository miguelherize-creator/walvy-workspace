...
Primera lectura (semáforo). Luego tabs. No cierra perfil financiero.
...
| 6 | `M1-V48` | `PATCH /auth/onboarding/step` | CTA a Inicio | Va a `/(tabs)`. Step body viejo; backend no marca `completed` solo con esto (`financialProfileCompleted` sigue false) | Pendiente |
